...
class PygameVisualizer:
    def __init__(self, env, agents, resources, goals, user_colors, width=700, height=700, cell_size=10):
        pygame.init()
        self.env = env
        self.agents = agents
        ...
        for entity in self.agents + self.resources + self.goals:
            if entity.name and entity.name not in self.colors:
                ...
                if entity.color is not None:
                    ...
                    self.colors[entity.name] = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
